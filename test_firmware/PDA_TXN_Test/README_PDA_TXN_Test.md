...
Прошивка без дисплея и радио. Опрашивает **CH0** (TXN @ `0x80`) как боевой `PDA_ESP32.ino`.
...
- EEPROM на **CH0** — тот же чип, что видит касса (кабель SDA/SCL или чип в слоте)
- EEPROM на **CH2** — слот снаряжения (только ping в SCAN)
...
| Команда | Действие |
|---------|----------|
| `SCAN` | TCA + EEPROM на CH0/CH2 |
| `DUMP` | TXN-блок на CH0 |
